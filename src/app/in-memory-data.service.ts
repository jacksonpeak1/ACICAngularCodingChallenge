...
export class InMemoryDataService implements InMemoryDbService {
  createDb() {
    ...
    recentQuotes.forEach((quote: recentQuote) => {
      const current: number | undefined = map.get(quote.lineOfBusiness);

      if (current) {
        map.set(quote.lineOfBusiness, current + 1);
      } else {
        map.set(quote.lineOfBusiness, 1);
      }
    });

    const linesOfBusiness: linesOfBusiness[] = [
      {
        id: 11,
    ...
    const mappedLinesOfBusiness = linesOfBusiness.map((lines) => {
      return {
        ...lines,
        quoteCount: map.get(lines.id),
      };
    });

    const sortedLinesOfBusiness = mappedLinesOfBusiness.sort(function (a, b) {
      return b.quoteCount - a.quoteCount;
    });

    return { linesOfBusiness: sortedLinesOfBusiness };
  }
